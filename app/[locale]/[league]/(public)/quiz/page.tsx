...
import { type FreezeState, resolveStreakFreeze } from "@/lib/streak-freeze";
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/${league}/quiz` },
    openGraph: {
      title: t("ogTitle"),
...
export default async function QuizPage({
  ...
  // The [league] layout redirects unknown/non-live slugs to the catalog, so a
  // resolved competition is guaranteed here; guard for types only.
  ...
  // Signed-in: this competition's answers (for streak + points) and this
  // question's existing answer (for the card). Anonymous: skip both.
  ...
  if (user) {
    ...
    const activityDays = new Set(answeredAt.map((iso) => new Date(iso).toISOString().slice(0, 10)));
    freeze = await resolveStreakFreeze(supabase, user.id, "quiz", activityDays, now);
  ...
  // Share standing for the signed-in viewer's own quiz streak in this league.
  // Read from the same public view the share card uses, so the text matches.
  const quizShare =
    user && answeredCount > 0 ? await loadQuizStanding(supabase, user.id, competition.id) : null;
    ...
    <main className="mx-auto max-w-2xl px-4 py-10">
      ...
      {user && quizShare ? (
        <section className="mb-8 rounded-xl border border-border bg-card p-4">
          ...
          <ShareButtons
            context="quiz"
            shareUrl={`${env.siteUrl}${buildQuizSharePath(locale, league, user.id)}`}
          ...
          <ol className="overflow-hidden rounded-xl border border-border bg-card">
            {rows.map((r, i) => {
              ...
              return (
                ...
                >
                  <span className="w-6 font-mono tabular-nums text-muted-foreground">{r.rank}</span>

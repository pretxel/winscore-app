...
Rules governing the persistent, competition-scoped section navigation rendered from `[league]/layout.tsx`. It ties a competition's sections (Matches, Standings, Leaderboard, Quiz, Bracket) together so each is discoverable and reachable from one consistent place on every page beneath a competition.

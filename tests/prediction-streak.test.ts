...
describe("computePredictionStreak", () => {
  ...
  it("counts only the most recent unbroken run", () => {
    ...
    expect(computePredictionStreak([day("06"), day("05"), day("03"), day("02")], NOW)).toBe(2);
  });

  it("dedupes multiple predictions on the same UTC day", () => {
    expect(
      computePredictionStreak([day("06", "01:00:00Z"), day("06", "23:00:00Z"), day("05")], NOW),
    ).toBe(2);
  });

  it("normalizes non-UTC offsets to the UTC day", () => {
    // 2026-06-07T01:00:00+03:00 === 2026-06-06T22:00:00Z → counts as today (06)
    expect(computePredictionStreak(["2026-06-07T01:00:00+03:00", day("05")], NOW)).toBe(2);
  });
  ...
  it("resets to 0 on a fresh Monday with no pick yet this week", () => {
    ...
    expect(computePredictionStreak(["2026-06-07T08:00:00Z", "2026-06-06T08:00:00Z"], monday)).toBe(
      0,
    );
  });
  ...
  it("caps the streak at the days elapsed in the week (<= 7)", () => {
    ...
    const everyDay = ["01", "02", "03", "04", "05", "06", "07"].map((d) => day(d));
  ...
  it("no freeze days reproduces current behavior exactly", () => {
    expect(computePredictionStreak([day("06"), day("05"), day("04")], NOW, new Set())).toBe(3);
  });

  it("a single forgiven gap inside the week keeps the streak alive", () => {
    // picks on 06 (today) and 04, gap on 05 forgiven → 3
    expect(computePredictionStreak([day("06"), day("04")], NOW, new Set(["2026-06-05"]))).toBe(3);
  });

  it("a two-day gap with one freeze still breaks", () => {
    expect(computePredictionStreak([day("06"), day("03")], NOW, new Set(["2026-06-05"]))).toBe(1);
  });

  it("a freeze never invents activity at the run end", () => {
    expect(computePredictionStreak([day("06")], NOW, new Set(["2026-06-05"]))).toBe(1);
  });
  ...
  it("a freeze cannot bridge into the prior week (weekly cap preserved)", () => {
    ...
    const monday = new Date("2026-06-08T12:00:00Z");
    expect(computePredictionStreak(["2026-06-08T08:00:00Z"], monday, new Set(["2026-06-07"]))).toBe(
      1,
    );
  });
});

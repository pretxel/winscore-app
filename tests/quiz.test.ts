...
describe("computeStreak", () => {
  ...
  it("dedupes multiple answers on the same UTC day", () => {
    expect(computeStreak([day("06", "01:00:00Z"), day("06", "23:00:00Z"), day("05")], NOW)).toBe(2);
  });
  ...
  it("no freeze days reproduces current behavior exactly", () => {
    expect(computeStreak([day("06"), day("05"), day("04")], NOW, new Set())).toBe(3);
...
describe("localizeQuizQuestion", () => {
  ...
  it("falls back on malformed translations JSON shapes", () => {
    ...
    expect(localizeQuizQuestion({ ...en, translations: "nope" }, "es")).toEqual(en);
    expect(localizeQuizQuestion({ ...en, translations: { es: 42 } }, "es")).toEqual(en);
    expect(localizeQuizQuestion({ ...en, translations: { es: { prompt: "P" } } }, "es")).toEqual(
      en,
    );
  });
});
